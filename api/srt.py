...
TRAIN_NAME = {
    "00": "KTX", "02": "무궁화", "03": "통근열차", "04": "누리로", "05": "전체",
    "07": "KTX-산천", "08": "ITX-새마을", "09": "ITX-청춘", "10": "KTX-산천",
    "17": "SRT", "18": "ITX-마음",
}
...
class SRTLoginError(SRTError): pass
class SRTResponseError(SRTError): pass
class SRTDuplicateError(SRTResponseError): pass
class SRTNotLoggedInError(SRTError): pass
class SRTNetFunnelError(SRTError): pass
...
class Passenger(metaclass=abc.ABCMeta):
    ...
    def __init_internal__(self, name: str, type_code: str, count: int):
        ...
        self.count = count
    def __repr__(self) -> str: return f"{self.name} {self.count}명"
    def __add__(self, other: "Passenger") -> "Passenger":
        if not isinstance(other, self.__class__): raise TypeError("Passenger types must be the same")
        if self.type_code == other.type_code: return self.__class__(count=self.count + other.count)
        raise ValueError("Passenger types must be the same")
    @classmethod
    def combine(cls, passengers: List["Passenger"]) -> List["Passenger"]:
        if not all(isinstance(p, Passenger) for p in passengers): raise TypeError("All passengers must be based on Passenger")
        passenger_dict = {}
    ...
    @staticmethod
    def get_passenger_dict(passengers: List["Passenger"], special_seat: bool = False, window_seat: str = None) -> Dict[str, str]:
        if not all(isinstance(p, Passenger) for p in passengers): raise TypeError("All passengers must be instances of Passenger")
        combined_passengers = Passenger.combine(passengers)
        data = {"totPrnb": Passenger.total_count(combined_passengers), "psgGridcnt": str(len(combined_passengers)), "locSeatAttCd1": WINDOW_SEAT.get(window_seat, "000"), "rqSeatAttCd1": "015", "dirSeatAttCd1": "009", "smkSeatAttCd1": "000", "etcSeatAttCd1": "000", "psrmClCd1": "2" if special_seat else "1",}
...
class SRTTicket:
    ...
    PASSENGER_TYPE = {"1": "어른/청소년", "2": "장애 1~3급", "3": "장애 4~6급", "4": "경로", "5": "어린이",}
    DISCOUNT_TYPE = {"000": "어른/청소년", "101": "탄력운임기준할인", "105": "자유석 할인", "106": "입석 할인", "107": "역방향석 할인", "108": "출입구석 할인", "109": "가족석 일반전환 할인", "111": "구간별 특정운임", "112": "열차별 특정운임", "113": "구간별 비율할인(기준)", "114": "열차별 비율할인(기준)", "121": "공항직결 수색연결운임", "131": "구간별 특별할인(기준)", "132": "열차별 특별할인(기준)", "133": "기본 특별할인(기준)", "191": "정차역 할인", "192": "매체 할인", "201": "어린이", "202": "동반유아 할인", "204": "경로", "205": "1~3급 장애인", "206": "4~6급 장애인",}
    ...
    def __str__(self) -> str: return self.dump()
    __repr__ = __str__
    def dump(self, format="string"):
        if format == "json": return vars(self)
        if self.is_waiting: return (f"예약대기 ({self.seat_type}) {self.passenger_type}" f"[{self.price}원({self.discount}원 할인)]")
        return (f"{self.car}호차 {self.seat} ({self.seat_type}) {self.passenger_type} " f"[{self.price}원({self.discount}원 할인)]")
...
class SRTReservation:
    ...
    def dump(self, format="string"):
        if format == "json":
            ...
            return data
        base = (f"[{self.train_name}] " f"{self.dep_date[4:6]}월 {self.dep_date[6:8]}일, " f"{self.dep_station_name}~{self.arr_station_name}" f"({self.dep_time[:2]}:{self.dep_time[2:4]}~{self.arr_time[:2]}:{self.arr_time[2:4]}) " f"{self.total_cost}원({self.seat_count}석)")
        if not self.paid:
            if not self.is_waiting: base += (f", 구입기한 {self.payment_date[4:6]}월 {self.payment_date[6:8]}일 " f"{self.payment_time[:2]}:{self.payment_time[2:4]}")
            elif not self.is_running: base += ", 예약대기"
        if self.is_running: base += f" (운행중)"
        return base
    @property
    def tickets(self): return self._tickets
...
class SRTResponseData:
    ...
    def __str__(self) -> str: return json.dumps(self._json)
    dump = __str__
    def _parse(self) -> dict:
        if "resultMap" in self._json: return self._json["resultMap"][0]
        if "ErrorCode" in self._json and "ErrorMsg" in self._json: raise SRTResponseError(f'Undefined result status "[{self._json["ErrorCode"]}]: {self._json["ErrorMsg"]}"')
        raise SRTError(f"Unexpected case [{self._json}]")
    def success(self) -> bool:
        result = self._status.get("strResult")
        if result is None: raise SRTResponseError("Response status is not given")
        if result == self.STATUS_SUCCESS: return True
        if result == self.STATUS_FAIL: return False
        raise SRTResponseError(f'Undefined result status "{result}"')
    def message(self) -> str: return self._status.get("msgTxt", "")
    def get_all(self) -> dict: return self._json.copy()
    def get_status(self) -> dict: return self._status.copy()
...
class SRTTrain(Train):
    ...
    def dump(self, format="string"):
        ...
        msg = (f"{train_line:<11s}" f"{month}/{day}" f" {dep_hour}:{dep_min}~{arr_hour}:{arr_min}  " f"{self.dep_station_name}~{self.arr_station_name}  " f"특실 {self.special_seat_state}, 일반실 {self.general_seat_state}")
        if self.reserve_wait_possible_code >= 0: msg += f", 예약대기 {self.reserve_wait_possible_name}"
        msg += f" ({duration:>3d}분)"
        return msg
    def general_seat_available(self): return "예약가능" in self.general_seat_state
    def special_seat_available(self): return "예약가능" in self.special_seat_state
    def reserve_standby_available(self): return self.reserve_wait_possible_code == 9
    def seat_available(self): return self.general_seat_available() or self.special_seat_available()
...
class NetFunnelHelper:
    ...
    DEFAULT_HEADERS = {"Host": "nf.letskorail.com", "Connection": "keep-alive", "Pragma": "no-cache", "Cache-Control": "no-cache", "sec-ch-ua-platform": "Android", "User-Agent": USER_AGENT, "sec-ch-ua": '"Chromium";v="136", "Android WebView";v="136", "Not=A/Brand";v="99"', "sec-ch-ua-mobile": "?1", "Accept": "*/*", "X-Requested-With": "kr.co.srail.newapp", "Sec-Fetch-Site": "cross-site", "Sec-Fetch-Mode": "no-cors", "Sec-Fetch-Dest": "script", "Sec-Fetch-Storage-Access": "active", "Referer": "https://app.srail.or.kr/", "Accept-Encoding": "gzip, deflate, br, zstd", "Accept-Language": "en-US,en;q=0.9,ko-KR;q=0.8,ko;q=0.7",}
    def __init__(self, debug=False):
        if HAS_CURL_CFFI: self._session = curl_cffi.Session(impersonate="chrome")
        else: self._session = requests.session()
    ...
    def run(self):
        ...
        try:
            ...
            if status in (self.WAIT_STATUS_PASS, self.ALREADY_COMPLETED): return self._cached_key
            self.clear()
    ...
    def _check(self, ip: str | None = None): return self._make_request("chkEnter", ip)
    def _complete(self, ip: str | None = None): return self._make_request("setComplete", ip)
    ...
    def _build_params(self, opcode: str, timestamp: str = None, key: str = None) -> dict:
        params = {"opcode": opcode, "nfid": "0", "prefix": f"NetFunnel.gRtype={opcode};", "js": "true", str(int(time.time() * 1000) if timestamp is None else timestamp): "",}
    ...
    def _parse(self, response: str) -> dict:
        ...
        if not result_match: raise SRTNetFunnelError("Failed to parse NetFunnel response")
        code, status, params_str = result_match.group(1).split(":", 2)
        if not params_str: raise SRTNetFunnelError("Failed to parse NetFunnel response")
...
class SRT:
    def __init__(self, srt_id: str, srt_pw: str, auto_login: bool = True, verbose: bool = False) -> None:
        if HAS_CURL_CFFI: self._session = curl_cffi.Session(impersonate="chrome")
        else: self._session = requests.session()
    ...
    def login(self, srt_id: str | None = None, srt_pw: str | None = None) -> bool:
        ...
        login_type = ("2" if EMAIL_REGEX.match(srt_id) else ("3" if PHONE_NUMBER_REGEX.match(srt_id) else "1"))
        if login_type == "3": srt_id = re.sub("-", "", srt_id)
        data = {"auto": "Y", "check": "Y", "page": "menu", "deviceKey": "-", "customerYn": "", "login_referer": API_ENDPOINTS["main"], "srchDvCd": login_type, "srchDvNm": srt_id, "hmpgPwdCphd": srt_pw,}
        r = self._session.post(url=API_ENDPOINTS["login"], data=data)
        self._log(r.text)
        if "존재하지않는 회원입니다" in r.text: raise SRTLoginError(r.json()["MSG"])
        if "비밀번호 오류" in r.text: raise SRTLoginError(r.json()["MSG"])
        if "Your IP Address Blocked" in r.text: raise SRTLoginError(r.text.strip())
        self.is_login = True
    ...
    def logout(self) -> bool:
        if not self.is_login: return True
        r = self._session.post(url=API_ENDPOINTS["logout"])
        self._log(r.text)
        if not r.ok: raise SRTResponseError(r.text)
        self.is_login = False
        self.membership_number = None
        return True
    ...
    def search_train(
        ...
        data = {
            "chtnDvCd": "1", "dptDt": date, "dptTm": time, "dptDt1": date,
            "dptTm1": time[:2] + "0000", "dptRsStnCd": STATION_CODE[dep],
            "arvRsStnCd": STATION_CODE[arr], "stlbTrnClsfCd": "05",
            "trnGpCd": 109, "trnNo": "", "psgNum": str(Passenger.total_count(passengers)),
            "seatAttCd": "015", "arriveTime": "N", "tkDptDt": "", "tkDptTm": "",
            "tkTrnNo": "", "tkTripChgFlg": "", "dlayTnumAplFlg": "Y",
        ...
        if time_limit:
            return [train for train in trains_from_api if train.dep_time <= time_limit]
            
        return trains_from_api

    def reserve(self, train: SRTTrain, passengers: list[Passenger] | None = None, option: SeatType = SeatType.GENERAL_FIRST, window_seat: bool | None = None) -> SRTReservation:
    ...
    def reserve_standby(self, train: SRTTrain, passengers: list[Passenger] | None = None, option: SeatType = SeatType.GENERAL_FIRST, mblPhone: str | None = None) -> SRTReservation:
        if option == SeatType.SPECIAL_FIRST: option = SeatType.SPECIAL_ONLY
        elif option == SeatType.GENERAL_FIRST: option = SeatType.GENERAL_ONLY
        return self._reserve(RESERVE_JOBID["STANDBY"], train, passengers, option, mblPhone=mblPhone)
    def _reserve(self, jobid: str, train: SRTTrain, passengers: list[Passenger] | None = None, option: SeatType = SeatType.GENERAL_FIRST, mblPhone: str | None = None, window_seat: bool | None = None) -> SRTReservation:
        if not self.is_login: raise SRTNotLoggedInError()
        if not isinstance(train, SRTTrain): raise TypeError('"train" must be SRTTrain instance')
        if train.train_name != "SRT": raise ValueError(f'Expected "SRT" train, got {train.train_name}')
        passengers = Passenger.combine(passengers or [Adult()])
        is_special_seat = {SeatType.GENERAL_ONLY: False, SeatType.SPECIAL_ONLY: True, SeatType.GENERAL_FIRST: not train.general_seat_available(), SeatType.SPECIAL_FIRST: train.special_seat_available(),}[option]
        data = {"jobId": jobid, "jrnyCnt": "1", "jrnyTpCd": "11", "jrnySqno1": "001", "stndFlg": "N", "trnGpCd1": "300", "trnGpCd": "109", "grpDv": "0", "rtnDv": "0", "stlbTrnClsfCd1": train.train_code, "dptRsStnCd1": train.dep_station_code, "dptRsStnCdNm1": train.dep_station_name, "arvRsStnCd1": train.arr_station_code, "arvRsStnCdNm1": train.arr_station_name, "dptDt1": train.dep_date, "dptTm1": train.dep_time, "arvTm1": train.arr_time, "trnNo1": f"{int(train.train_number):05d}", "runDt1": train.dep_date, "dptStnConsOrdr1": train.dep_station_constitution_order, "arvStnConsOrdr1": train.arr_station_constitution_order, "dptStnRunOrdr1": train.dep_station_run_order, "arvStnRunOrdr1": train.arr_station_run_order, "mblPhone": mblPhone, "netfunnelKey": self._netfunnel.run(),}
        ...
        self._log(r.text)
        parser = SRTResponseData(r.text)
        if not parser.success(): raise SRTResponseError(parser.message())
        reservation_number = parser.get_all()["reservListMap"][0]["pnrNo"]
        for ticket in self.get_reservations():
            if ticket.reservation_number == reservation_number: return ticket
        raise SRTError("Ticket not found: check reservation status")
    def reserve_standby_option_settings(self, reservation: SRTReservation | int, isAgreeSMS: bool, isAgreeClassChange: bool, telNo: str | None = None) -> bool:
        if not self.is_login: raise SRTNotLoggedInError()
        reservation_number = getattr(reservation, "reservation_number", reservation)
        data = {"pnrNo": reservation_number, "psrmClChgFlg": "Y" if isAgreeClassChange else "N", "smsSndFlg": "Y" if isAgreeSMS else "N", "telNo": telNo if isAgreeSMS else "",}
    ...
    def get_reservations(self, paid_only: bool = False) -> list[SRTReservation]:
        ...
        parser = SRTResponseData(r.text)
        if not parser.success(): raise SRTResponseError(parser.message())
        return [SRTReservation(train, pay, self.ticket_info(train["pnrNo"])) for train, pay in zip(parser.get_all()["trainListMap"], parser.get_all()["payListMap"]) if not paid_only or pay["stlFlg"] != "N"]
    def ticket_info(self, reservation: SRTReservation | int) -> list[SRTTicket]:
        if not self.is_login: raise SRTNotLoggedInError()
        reservation_number = getattr(reservation, "reservation_number", reservation)
        r = self._session.post(url=API_ENDPOINTS["ticket_info"], data={"pnrNo": reservation_number, "jrnySqno": "1"},)
        self._log(r.text)
        parser = SRTResponseData(r.text)
        if not parser.success(): raise SRTResponseError(parser.message())
    ...
    def cancel(self, reservation: SRTReservation | int) -> bool:
        ...
        if not parser.success(): raise SRTResponseError(parser.message())
        return True
    def pay_with_card(self, reservation: SRTReservation, number: str, password: str, validation_number: str, expire_date: str, installment: int = 0, card_type: str = "J",) -> bool:
        if not self.is_login: raise SRTNotLoggedInError()
        data = {"stlDmnDt": datetime.now().strftime("%Y%m%d"), "mbCrdNo": self.membership_number, "stlMnsSqno1": "1", "ststlGridcnt": "1", "totNewStlAmt": reservation.total_cost, "athnDvCd1": card_type, "vanPwd1": password, "crdVlidTrm1": expire_date, "stlMnsCd1": "02", "rsvChgTno": "0", "chgMcs": "0", "ismtMnthNum1": installment, "ctlDvCd": "3102", "cgPsId": "korail", "pnrNo": reservation.reservation_number, "totPrnb": reservation.seat_count, "mnsStlAmt1": reservation.total_cost, "crdInpWayCd1": "@", "athnVal1": validation_number, "stlCrCrdNo1": number, "jrnyCnt": "1", "strJobId": "3102", "inrecmnsGridcnt": "1", "dptTm": reservation.dep_time, "arvTm": reservation.arr_time, "dptStnConsOrdr2": "000000", "arvStnConsOrdr2": "000000", "trnGpCd": "300", "pageNo": "-", "rowCnt": "-", "pageUrl": "",}
        r = self._session.post(url=API_ENDPOINTS["payment"], data=data)
        self._log(r.text)
        response = json.loads(r.text)
        if response["outDataSets"]["dsOutput0"][0]["strResult"] == "FAIL": raise SRTResponseError(response["outDataSets"]["dsOutput0"][0]["msgTxt"])
        return True
    def reserve_info(self, reservation: SRTReservation | int) -> bool:
        ...
        self._log(r.text)
        response = json.loads(r.text)
        if response.get("ErrorCode") == "0" and response.get("ErrorMsg") == "": return response.get("outDataSets").get("dsOutput1")[0]
        else: raise SRTResponseError(response.get("ErrorMsg"))
    def refund(self, reservation: SRTReservation | int) -> bool:
        info = self.reserve_info(reservation)
        data = {"pnr_no": info.get("pnrNo"), "cnc_dmn_cont": "승차권 환불로 취소", "saleDt": info.get("ogtkSaleDt"), "saleWctNo": info.get("ogtkSaleWctNo"), "saleSqno": info.get("ogtkSaleSqno"), "tkRetPwd": info.get("ogtkRetPwd"), "psgNm": info.get("buyPsNm"),}
        r = self._session.post(url=API_ENDPOINTS["refund"], data=data)
        self._log(r.text)
        response = SRTResponseData(r.text)
        if not response.success(): raise SRTResponseError(response.message())
        return True
    def clear(self):
