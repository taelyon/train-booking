...
DEFAULT_HEADERS = {"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8", "User-Agent": USER_AGENT, "Host": "smart.letskorail.com", "Connection": "Keep-Alive", "Accept-Encoding": "gzip",}
KORAIL_MOBILE = "https://smart.letskorail.com:443/classes/com.korail.mobile"
API_ENDPOINTS = {"login": f"{KORAIL_MOBILE}.login.Login", "logout": f"{KORAIL_MOBILE}.common.logout", "search_schedule": f"{KORAIL_MOBILE}.seatMovie.ScheduleView", "reserve": f"{KORAIL_MOBILE}.certification.TicketReservation", "cancel": f"{KORAIL_MOBILE}.reservationCancel.ReservationCancelChk", "myticketseat": f"{KORAIL_MOBILE}.refunds.SelTicketInfo", "myticketlist": f"{KORAIL_MOBILE}.myTicket.MyTicketList", "myreservationview": f"{KORAIL_MOBILE}.reservation.ReservationView", "myreservationlist": f"{KORAIL_MOBILE}.certification.ReservationList", "pay": f"{KORAIL_MOBILE}.payment.ReservationPayment", "refund": f"{KORAIL_MOBILE}.refunds.RefundsRequest", "code": f"{KORAIL_MOBILE}.common.code.do",}
...
class Schedule:
    ...
    def __repr__(self):
        ...
        return (f"{train_line:<11s}" f"{dep_date} {dep_time}~{arr_time}  " f"{self.dep_name}~{self.arr_name}")
class Train(Schedule):
    def __init__(self, data):
        ...
        if self.wait_reserve_flag: self.wait_reserve_flag = int(self.wait_reserve_flag)
    def __repr__(self):
        ...
        if self.reserve_possible_name:
            ...
            if self.wait_reserve_flag >= 0: repr_str += f", 예약대기 {'가능' if self.has_general_waiting_list() else '매진'}"
        repr_str += f" ({duration:>3d}분)"
        return repr_str
    def has_special_seat(self): return self.special_seat == "11"
    def has_general_seat(self): return self.general_seat == "11"
    def has_seat(self): return self.has_general_seat() or self.has_special_seat()
    def has_waiting_list(self): return self.has_general_waiting_list()
    def has_general_waiting_list(self): return self.wait_reserve_flag == 9
class Ticket(Train):
    def __init__(self, data):
    ...
    def __repr__(self):
        ...
        if int(self.seat_no_count) != 1: repr_str += f" {self.seat_no}~{self.seat_no_end}"
        else: repr_str += f" {self.seat_no}"
        repr_str += f", {self.price}원"
        return repr_str
    def get_ticket_no(self): return "-".join(map(str, (self.sale_info1, self.sale_info2, self.sale_info3, self.sale_info4),))
class Reservation(Train):
    ...
    def __repr__(self):
        ...
        if self.is_waiting: repr_str += ", 예약대기"
        else:
...
class Seat:
    ...
    def __repr__(self):
        ...
        else: return (f"{self.car}호차 {self.seat} ({self.seat_type}) {self.passenger_type} " f"[{self.price}원({self.discount}원 할인)]")
class Passenger:
    ...
    def reduce(passenger_list):
        if not all(isinstance(x, Passenger) for x in passenger_list): raise TypeError("Passengers must be based on Passenger")
    ...
    def __add__(self, other):
        ...
        if self.group_key() != other.group_key(): raise TypeError(f"Cannot add passengers with different group keys: {self.group_key()} vs {other.group_key()}")
    ...
    def group_key(self): return f"{self.typecode}_{self.discount_type}_{self.card}_{self.card_no}_{self.card_pw}"
    def get_dict(self, index):
        index = str(index)
        return {f"txtPsgTpCd{index}": self.typecode, f"txtDiscKndCd{index}": self.discount_type, f"txtCompaCnt{index}": self.count, f"txtCardCode_{index}": self.card, f"txtCardNo_{index}": self.card_no, f"txtCardPw_{index}": self.card_pw,}
...
class NetFunnelHelper:
    ...
    def __init__(self):
        if HAS_CURL_CFFI: self._session = curl_cffi.Session(impersonate="chrome131_android")
    ...
    def _parse(self, response: str) -> dict:
        ...
        if not params_str: raise NetFunnelError("Failed to parse NetFunnel response")
...
class Korail:
    ...
    def __init__(self, korail_id, korail_pw, auto_login=True, verbose=False):
        ...
        if not Korail._STATION_MAP:
            Korail._STATION_MAP = self.get_station_codes()
    ...
    @staticmethod
    def get_station_codes(name_only=False):
        # This is a mock implementation based on the original library's logic.
        # In a real scenario, this might need to fetch data from a server.
        stations = {
            "서울": "0001", "용산": "0002", "영등포": "0003", "광명": "0004", "수원": "0005", "천안아산": "0006",
            "오송": "0007", "대전": "0008", "서대전": "0009", "김천구미": "0010", "동대구": "0011", "경주": "0012",
            "포항": "0013", "밀양": "0014", "구포": "0015", "부산": "0016", "울산(통도사)": "0017", "마산": "0018",
            "창원중앙": "0019", "경산": "0020", "논산": "0021", "익산": "0022", "정읍": "0023", "광주송정": "0024",
            "목포": "0025", "전주": "0026", "순천": "0027", "여수EXPO": "0028", "청량리": "0029", "강릉": "0030", "행신": "0031"
        }
        if name_only:
    ...
    def search_train(
        ...
        dep_code = self._STATION_MAP.get(dep, dep)
        arr_code = self._STATION_MAP.get(arr, arr)
        ...
        data = {
            "Device": self._device, "Version": self._version, "Sid": "",
            "txtMenuId": "11", "radJobId": "1", "selGoTrain": train_type,
            "txtTrnGpCd": train_type, "txtGoStart": dep_code, "txtGoEnd": arr_code,
            ...
            "ebizCrossCheck": "N", "srtCheckYn": "N", "rtYn": "N",
            "adjStnScdlOfrFlg": "N", "mbCrdNo": self.membership_number,
        }
    ...
    def reserve(self, train, passengers=None, option=ReserveOption.GENERAL_FIRST):
        ...
        if reserving_seat:
            ...
            is_special_seat = {ReserveOption.GENERAL_ONLY: False, ReserveOption.SPECIAL_ONLY: True, ReserveOption.GENERAL_FIRST: False, ReserveOption.SPECIAL_FIRST: True,}[option]
        passengers = passengers or [AdultPassenger()]
        passengers = Passenger.reduce(passengers)
        cnt = sum(p.count for p in passengers)
        data = {"Device": self._device, "Version": self._version, "Key": self._key, "txtMenuId": "11", "txtJobId": "1101" if reserving_seat else "1102", "txtGdNo": "", "hidFreeFlg": "N", "txtTotPsgCnt": cnt, "txtSeatAttCd1": "000", "txtSeatAttCd2": "000", "txtSeatAttCd3": "000", "txtSeatAttCd4": "015", "txtSeatAttCd5": "000", "txtStndFlg": "N", "txtSrcarCnt": "0", "txtJrnyCnt": "1", "txtJrnySqno1": "001", "txtJrnyTpCd1": "11", "txtDptDt1": train.dep_date, "txtDptRsStnCd1": train.dep_code, "txtDptTm1": train.dep_time, "txtArvRsStnCd1": train.arr_code, "txtTrnNo1": train.train_no, "txtRunDt1": train.run_date, "txtTrnClsfCd1": train.train_type, "txtTrnGpCd1": train.train_group, "txtPsrmClCd1": "2" if is_special_seat else "1", "txtChgFlg1": "", "txtJrnySqno2": "", "txtJrnyTpCd2": "", "txtDptDt2": "", "txtDptRsStnCd2": "", "txtDptTm2": "", "txtArvRsStnCd2": "", "txtTrnNo2": "", "txtRunDt2": "", "txtTrnClsfCd2": "", "txtPsrmClCd2": "", "txtChgFlg2": "",}
    ...
    def tickets(self):
        data = {"Device": self._device, "Version": self._version, "Key": self._key, "txtDeviceId": "", "txtIndex": "1", "h_page_no": "1", "h_abrd_dt_from": "", "h_abrd_dt_to": "", "hiduserYn": "Y",}
        ...
        try:
            if self._result_check(j):
                ...
                for info in j.get("reservation_list", []):
                    ticket = Ticket(info)
                    data = {"Device": self._device, "Version": self._version, "Key": self._key, "h_orgtk_wct_no": ticket.sale_info1, "h_orgtk_ret_sale_dt": ticket.sale_info2, "h_orgtk_sale_sqno": ticket.sale_info3, "h_orgtk_ret_pwd": ticket.sale_info4,}
                ...
                return tickets
        except NoResultsError: return []
    def reservations(self, rsv_id=None):
        data = {"Device": self._device, "Version": self._version, "Key": self._key,}
    ...
    def ticket_info(self, rsv_id=None):
        data = {"Device": self._device, "Version": self._version, "Key": self._key, "hidPnrNo": rsv_id,}
        ...
        try:
            ...
            if jrny_info := j.get("jrny_infos", {}).get("jrny_info", []):
                if seat_info := jrny_info[0].get("seat_infos", {}).get("seat_info", []): return [Seat(seat) for seat in seat_info], wct_no
        except NoResultsError: return None
    def pay_with_card(self, rsv, card_number, card_password, birthday, card_expire, installment=0, card_type="J",):
        if not isinstance(rsv, Reservation): raise TypeError("rsv must be a Reservation instance")
        ...
        self._log(r.text)
        j = json.loads(r.text)
        if self._result_check(j): return True
        return False
    def cancel(self, rsv):
        if not isinstance(rsv, Reservation): raise TypeError("rsv must be a Reservation instance")
        data = {"Device": self._device, "Version": self._version, "Key": self._key, "txtPnrNo": rsv.rsv_id, "txtJrnySqno": rsv.journey_no, "txtJrnyCnt": rsv.journey_cnt, "hidRsvChgNo": rsv.rsv_chg_no,}
        r = self._session.post(API_ENDPOINTS["cancel"], data=data)
        self._log(r.text)
        j = json.loads(r.text)
        return self._result_check(j)
    def refund(self, ticket):
        data = {"Device": self._device, "Version": self._version, "Key": self._key, "txtPrnNo": ticket.pnr_no, "h_orgtk_sale_dt": ticket.sale_info2, "h_orgtk_sale_wct_no": ticket.sale_info1, "h_orgtk_sale_sqno": ticket.sale_info3, "h_orgtk_ret_pwd": ticket.sale_info4, "h_mlg_stl": "N", "tk_ret_tms_dv_cd": "21", "trnNo": ticket.train_no, "pbpAcepTgtFlg": "N", "latitude": "", "longitude": "",}
